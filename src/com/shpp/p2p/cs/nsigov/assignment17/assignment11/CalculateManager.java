...
public class CalculateManager {
    ...
    /**
     * String to determine if the operator symbol matches
     */
    private static final String REGEX = "\\+?-?\\*?/?\\^?(sqrt)?(sin)?(cos)?(tan)?(atan)?(log10)?(log2)?";

    /**
     * The formula is parsed according to the Dijkstra's algorithm
     */
    private final MyArrayList<String> PARSED_FORMULA;
     ...
     */
    CalculateManager(MyArrayList<String> parsedFormula, MyHashMap<String, Double> parseVariables, String formula) {
    ...
    void calculate() {
        ...
        while (PARSED_FORMULA.size() != 0) {
            if (!PARSED_FORMULA.get(0).matches(REGEX)) {  // if operand
                stack.add(PARSED_FORMULA.get(0));
            } else { // if operator like sin.cos.tan(unary)
     ...
     */
    private double checkVariables(String s, MyHashMap<String, Double> variables) {
        boolean minus = false;
